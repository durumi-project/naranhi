...
**최종 업데이트: 2026-05-15 (세션 6 완료 — 두루 자료 추출 10건 추가, 누적 25건)**

```
✅ 작동하는 데모          localhost:5173에서 6단계 사용자 여정 + 안전 분기 정상
✅ GitHub 저장소           durumi-project/naranhi (비공개), 팀원 4명 합류 중
✅ 자료 25건                가상 10건 (검수완료) + OM2 3건 + 두루 추출 12건 (검수대기) — 갱신
✅ 진짜 판례 4건            DR-PREC-001/004/007/008 (첫 SX·첫 불인정·첫 민사 포함) — 갱신
✅ 첫 EX·첫 SX 사례        DR-CASE-003(갈취) + DR-PREC-004(성폭력) — 신규
✅ 검수 트랙               src/data/cases/PENDING ↔ REVIEWED 폴더 구조
✅ 자동 검증               스키마 v2 (42필드) + validateCase 함수 (25/25 통과)
✅ 두루 매뉴얼 5종         활용 가능 확인됨 (docs/external/)
✅ 두루 직접 제공 자료     50건 사례 + 판례 10건 (docs/external/, 활용 허락 받음)
✅ PDF 처리 인프라         poppler 설치, PDF 추출 준비 완료
🟡 시범 사례 추출          50건 측 8/50, 판례 측 4/10 (M1 30~50건 절반 진입)
🟡 두루 변호사 검수        PENDING 15건 검수 패키지 미작성
❌ LLM 통합                미시작 (자료 더 모인 후)
❌ 배포                    미시작 (localhost만)
❌ 학교 현장 노출          미시작
```

**한 줄로**: *동작하는 데모는 완성. 두루 자료에서 *진짜 사례 25건* 누적 — M1(30~50건) 절반 진입. 두루 변호사 검수 패키지 작성이 다음 가장 큰 과제.*
...
│   └── data/
│       ├── cases/
│       │   ├── PENDING/            ← 검수 대기 사례 (15건)
│       │   │   ├── DR-CASE-003.json (두루 50건 3번 — 금품갈취, 첫 EX) — 신규
│       │   │   ├── DR-CASE-004.json (두루 50건 4번 — 운동부 위계폭력, HS)
│       │   │   ├── DR-CASE-006.json (두루 50건 6번 — 특성 기반 괴롭힘, ES)
│       │   │   ├── DR-CASE-010.json (두루 50건 10번 — 미인지·미신고 외모 비하)
│       │   │   ├── DR-CASE-019.json (두루 50건 19번 — 장난 가장 신체, DR-PREC-001 짝)
│       │   │   ├── DR-CASE-020.json (두루 50건 20번 — 장기 따돌림 + 민사 검토)
│       │   │   ├── DR-CASE-022.json (두루 50건 22번 — 학원→학교 번짐)
│       │   │   ├── DR-CASE-047.json (두루 50건 47번 — 단톡방 강제 퇴장)
│       │   │   ├── DR-PREC-001.json (두루 판례 2024구합24300 — 첫 precedent)
│       │   │   ├── DR-PREC-004.json (두루 판례 2022구합20199 — 첫 SX, safety_flag=true) — 신규
│       │   │   ├── DR-PREC-007.json (두루 판례 2023구합21694 — 첫 *학폭 불인정*) — 신규
│       │   │   ├── DR-PREC-008.json (두루 판례 2022나66828 — 첫 민사 손해배상) — 신규
│       │   │   ├── OM2-IV-001.json
│       │   │   ├── OM2-IV-002.json (⚠️ 자해 언급, safety_flag=true)
│       │   │   └── OM2-IV-003.json
│       │   ├── REVIEWED/           ← 검수 완료 (가상 사례 10건)
