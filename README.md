...
✅ 자료 13건         가상 사례 10건 (검수완료) + 온마음2 갈등조정 사례 3건 (검수대기)
✅ 검수 트랙          src/data/cases/PENDING ↔ REVIEWED 폴더 구조
✅ 자동 검증          스키마 v2 (42필드) + validateCase 함수
🟡 두루 변호사 검수   미시작
🟡 자료 추가 수집     두루 매뉴얼·인천 모음집 활용 협의 필요
❌ LLM 통합           미시작 (자료 더 모인 후)
❌ 배포              미시작 (localhost만)
❌ 학교 현장 노출     미시작
```

**한 줄로**: *동작하는 데모는 완성*, *진짜 자료 수집·검수·LLM·배포는 다음 단계*.
...
# 변경 후
git add 파일경로
git commit -m "feat: 인천 모음집 사례 1건 추가 (PENDING)

- src/data/cases/PENDING/IN-001.json
- 스키마 v2 검증 통과
- 시나리오 A·B 동작 확인"
```
...
- Claude Code 안에서 `npm run dev` 띄우지 말 것 — 무한 대기 발생
...
3. **`docs/` PDF 4종** — 분류코드표·친화변환·검수체크리스트·인수인계.
4. **GitHub Issues** — 질문·이슈 등록. 다른 팀원이 답해줘요.
5. **카톡·디스코드** — 빠른 질문 (팀 채널 따로 있으면 거기).
...
| 역할 | 어떤 작업 | 필요 기술 |
|---|---|---|
| **데이터 큐레이터** | 새 사례 수집·추출 → PENDING/에 추가 | JSON 편집, 스키마 v2 이해 |
...
5. **첫 작은 PR** — 예: 이 README 오타 수정, 또는 본인 환경에서 발견한 작은 문제
...
- **두루 자료 활용 허락**: 두루 매뉴얼 4종 (152쪽 + 212쪽 + 128쪽 + 8쪽)의 외부 활용 가능 범위 두루 변호사 확인 필요.
- **인천교육청 모음집**: 「제2회 학교폭력 예방 우수사례 모음집」 수집 시도 필요.
...
각 단계는 *학기·연 단위*. 너무 멀게 보지 말고, *지금 한 걸음*에 집중.
